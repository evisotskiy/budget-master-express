...
router.get('/list/:categoryId', async (req, res) => {
    try {
        ...
        if (!category) {
            return res.status(404).json({ message: `Category with id = ${req.params.categoryId} is not found` })
        }

        let records = await Record.find({ userId, categoryId: req.params.categoryId });
...
router.patch('/:id', createUpdateValidators, checkValidationResults, async (req, res) => {
    try {
        ...
        const record = await Record.findOneAndUpdate({ id: req.params.id, userId }, formData, { runValidators: true, new: true })
