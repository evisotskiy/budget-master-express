...
const {
  ...
  getRecordSchema,
  getRecordListSchema,
} = require('./validationSchemas/record.schemas');
...
describe('Category routes test suit', () => {
  ...
  async function generateCategoriesAndRecords(categoriesAmount, recordsAmount) {
    ...
    try {
      ...
      for (let newCategory = null, i = 0; i < categoriesAmount; i++) {
        ...
        for (let newRecord = null, j = 0; j < recordsAmount; j++) {
  ...
  it(`create new record`, async () => {
    try {
      result = await app
        ...
        .send({ description, amount, date, type, categoryId: category.id });
    } catch (e) {
      console.error(e);
    }
  ...
  it(`get 422 if try to create record with invalid fields`, async () => {
    ...
    try {
      allResults = await Promise.all(
        payloadCombinations.map(
          async (payload) =>
            await app.post(`${routesBaseUri}/`).set('Authorization', `Bearer ${accessToken}`).send(payload),
        ),
      );
    } catch (e) {
      console.error(e);
  ...
  it(`update existing record`, async () => {
    ...
    try {
      ...
      result = await app.patch(`${routesBaseUri}/${record.id}`).set('Authorization', `Bearer ${accessToken}`).send({
  ...
  it(`get 404 if try to update nonexistent record`, async () => {
    ...
    try {
      ...
      result = await app
        .patch(`${routesBaseUri}/${nonexistentRecordId}`)
        .set('Authorization', `Bearer ${accessToken}`)
        .send({
          description: newDescription,
          amount: newAmount,
  ...
  it(`get 404 if try to delete nonexistent record`, async () => {
    ...
    try {
      ...
      result = await app
        .delete(`${routesBaseUri}/${nonexistentRecordId}`)
        .set('Authorization', `Bearer ${accessToken}`);
    } catch (e) {
      console.error(e);
    }
  ...
  it(`get all user's records by the certain category`, async () => {
    try {
      ...
      result = await app
        .get(`${routesBaseUri}/list/${allCategories[0].id}`)
        .set('Authorization', `Bearer ${accessToken}`);
    } catch (e) {
      console.error(e);
    }
  ...
  it(`get 404 if try to get records with the nonexistent category`, async () => {
    ...
    try {
      ...
      result = await app
        .get(`${routesBaseUri}/list/${nonexistentCategoryId}`)
        .set('Authorization', `Bearer ${accessToken}`);
    } catch (e) {
      console.error(e);
    }
